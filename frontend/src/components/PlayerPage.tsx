...
export default function PlayerPage() {
  ...
  function handleVolume(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (v) v.volume = val; setVolume(val); setMuted(val === 0);
  }
  function toggleMute() {
    const v = videoRef.current; if (!v) return;
  ...
  async function handleSaveTags() {
    if (!video) return; setSaving(true);
    try {
      const tags = tagInput.split(',').map(t => t.trim()).filter(Boolean);
      const updated = await updateVideoTags(video.id, tags);
      setVideo(updated); setTagInput(updated.tags.join(', '));
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
          <motion.button whileHover={{ x: -2 }} whileTap={{ scale: 0.92 }} onClick={() => navigate('/')}
        ...
        <a href={src} download={`${video.title}.mp4`}
          style={{
            ...
            fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', whiteSpace: 'nowrap',
          }}>
          {isMobile ? '↓' : 'Download'}
        </a>
      </header>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        >
          ...
          <AnimatePresence>
            {showControls && (
              ...
              >
                <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, fontWeight: 600, marginBottom: 5, letterSpacing: '0.02em' }}>
                  {video.channel}
                </p>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 11, fontWeight: 600 }}>{new Date(video.downloaded_at).getFullYear()}</span>
                  <span style={{ width: 3, height: 3, borderRadius: '50%', background: 'rgba(255,255,255,0.4)', display: 'inline-block' }} />
                  <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 11, fontWeight: 600 }}>{formatFileSize(video.file_size)}</span>
                ...
                <h1 style={{
                  ...
                  color: '#fff', textShadow: '0 2px 20px rgba(0,0,0,0.7)',
                  fontFamily: 'var(--font)',
                }}>
                  {video.title}
                </h1>
          ...
          <AnimatePresence>
            {showControls && (
              <motion.div key="centre"
                ...
                style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: isMobile ? 32 : 52, zIndex: 22, pointerEvents: 'none' }}
              >
                <motion.button whileTap={{ scale: 0.82 }} onClick={e => { e.stopPropagation(); handleSkip(-10); }}
          ...
          <AnimatePresence>
            {showControls && (
              <motion.div key="bar"
                ...
                style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: isMobile ? '0 14px 14px' : '0 28px 20px', zIndex: 30 }}
              >
                {/* Time labels */}
                ...
                <div style={{ position: 'relative', marginBottom: isMobile ? 10 : 14 }}>
                  <div style={{ width: '100%', height: isMobile ? 3 : 5, background: 'rgba(255,255,255,0.18)', borderRadius: 9999, overflow: 'hidden' }}>
                    <motion.div
                      style={{ height: '100%', background: 'var(--red)', borderRadius: 9999, boxShadow: '0 0 8px rgba(229,9,20,0.8)' }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', color: '#fff' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 10 : 20 }}>
                    ...
                    {/* Track title in controls */}
                    {!isMobile && (
                      <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.75)', fontWeight: 600, marginLeft: 4, maxWidth: 320, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {video.title}
                      </p>
                    )}
                  </div>
        ...
        {/* ── Mobile Up Next — horizontal scroll ────────────────────── */}
        {isMobile && related.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ ...spring, delay: 0.2 }}
            style={{ padding: '16px 16px 8px' }}>
            <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 10 }}>Up Next</p>
            <div style={{ display: 'flex', gap: 12, overflowX: 'auto', paddingBottom: 8 }}>
              {related.map(v => (
                <motion.div key={v.id} whileTap={{ scale: 0.96 }} onClick={() => navigate(`/player/${v.id}`)}
                  ...
                  <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>{v.channel}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
        ...
        <motion.div
          ...
          style={{ maxWidth: 900, width: '100%', margin: '0 auto', padding: isMobile ? '16px 16px 40px' : '24px 32px 48px' }}
        >
          {/* Title + meta */}
          <h2 style={{ fontSize: isMobile ? 18 : 22, fontWeight: 800, letterSpacing: '-0.03em', color: 'var(--text)', marginBottom: 6, fontFamily: 'var(--font)', lineHeight: 1.2 }}>
            {video.title}
          </h2>
          ...
          {/* More info toggle */}
          <motion.button whileTap={{ scale: 0.97 }} onClick={() => setShowInfo(p => !p)}
            style={{ color: 'var(--text-muted)', background: 'var(--surface)', border: 'none', borderRadius: 8, padding: '7px 14px', fontSize: 12, fontWeight: 600, cursor: 'pointer', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {/* Tag editor */}
          <div style={{ background: 'var(--surface)', borderRadius: 10, padding: '14px 18px', marginBottom: 14 }}>
